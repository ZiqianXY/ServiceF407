...
ErrorStatus RTC_Set_Date(u8 year,u8 month,u8 date,u8 week)
{
	...
	RTC_DateTypeInitStructure.RTC_Date=date;// 日期从1980年开始计算
...
u8 My_RTC_Init(void)
{
  ...
  RCC_APB1PeriphClockCmd(RCC_APB1Periph_PWR, ENABLE);//使能PWR时钟
    ...
    RTC_Init(&RTC_InitStructure);
		...
		RTC_Set_Time(10,14,56,RTC_H12_AM);	//设置时间
		RTC_Set_Date(16,6,5,7);		//设置日期
	 
		RTC_WriteBackupRegister(RTC_BKP_DR0,0x5555);	//标记已经初始化过了
	} 
...
void getTimeString(char* str){
	// 注意STM32的时间是2000-2099，fafts时间是1980-(1980+127)
	RTC_GetTime(RTC_Format_BIN,&RTC_TimeStruct);	
	RTC_GetDate(RTC_Format_BIN, &RTC_DateStruct);
